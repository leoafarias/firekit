...
> extends AbstractRepository<T> {
  ...
  /**
   * Get all entities as an array for query processing.
   * @private
  ...
  private _getStore(): { [collectionName: string]: Map<string, any> } {
    // Get the full store from the adapter
    if (
      "getStore" in this.adapter &&
      typeof this.adapter.getStore === "function"
    ) {
      return this.adapter.getStore();
    }

    // Fallback to creating a minimal store with just this collection
